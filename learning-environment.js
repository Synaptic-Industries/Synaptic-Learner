...
class LearningEnvironment {
    ...
    async initializeEnvironment() {
        ...
        // Wait for course content to load if needed
        let attempts = 0;
        while (!window.courseContent && attempts < 50) {
    ...
    async loadCourseData() {
        if (!window.courseContent || !window.courseContent[this.currentCourse]) {
            console.error('Course not found:', this.currentCourse);
            ...
            chapter.lessons.forEach(lessonId => {
                ...
                lessonItem.innerHTML = `
                    <div class="lesson-status">
                        ${this.isLessonCompleted(lessonId) ? 'DONE' : lessonId}
                    </div>
    ...
    async initializeMonacoEditor() {
        ...
        // Clear any existing content and show simple loading message
        editorContainer.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#888;font-size:14px;">Initializing editor...</div>';
    ...
    loadLesson(lessonId) {
        console.log('Loading lesson:', lessonId, 'for course:', this.currentCourse);
        const course = window.courseContent[this.currentCourse];
        console.log('Course data:', course);
        console.log('Available lessons:', course ? course.lessons.map(l => l.id) : 'No course');
    ...
    setupEventListeners() {
        ...
        // Studio simulator removed for simplicity
    ...
    testCode() {
        ...
        // Ensure output is visible after test
    ...
    addToOutput(text, type = 'normal') {
        ...
        // Force scroll to bottom to show latest output
        setTimeout(() => {
    ...
    ensureOutputVisible() {
        ...
        // Force immediate scroll to bottom without animation to prevent jumping
        outputContent.scrollTop = outputContent.scrollHeight;
    }
    ...
    markLessonCompleted(lessonId) {
        ...
        // Update only the specific lesson's completion status without rebuilding entire sidebar
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('DOM Content Loaded, initializing learning environment...');
    console.log('Course content available:', !!window.courseContent);
    if (window.courseContent) {
        console.log('Available courses:', Object.keys(window.courseContent));
    }
